...
public class WrapperPlayServerSpawnEntityLiving extends AbstractPacket {
    private static final ImmutableBiMap<EntityType, Integer> entityTypeIds;
    static {
        ImmutableBiMap<EntityType, Integer> entityTypeIds1;
        ImmutableBiMap.Builder<EntityType, Integer> mapBuilder = new ImmutableBiMap.Builder<>();
        String nmsVersion = Bukkit.getServer().getClass().getName().split("\\.")[3];
        try {
            Field entityTypeRegistryField = Class.forName("net.minecraft.server." + nmsVersion + ".IRegistry")
                    .getField("ENTITY_TYPE");
            entityTypeRegistryField.setAccessible(true);
            Object entityTypeRegistry = entityTypeRegistryField.get(null); // RegistryBlocks<EntityTypes<?>>
            Method getNetIdMethod = null;
            for (Method m : Class.forName("net.minecraft.server." + nmsVersion + ".Registry").getDeclaredMethods()) {
                if (m.getReturnType() != int.class) continue;
                getNetIdMethod = m;
                break;
            }
            if (getNetIdMethod == null) throw new NoSuchMethodException("Unable to find net id getter method");
            getNetIdMethod.setAccessible(true);
            Class<?> nmsEntityTypes = Class.forName("net.minecraft.server." + nmsVersion + ".EntityTypes");
            for (EntityType type : EntityType.values()) {
                if (type == EntityType.UNKNOWN) continue;
                String constantName = type.getKey().getKey().toUpperCase(Locale.ROOT);
                Field constantField = nmsEntityTypes.getDeclaredField(constantName);
                constantField.setAccessible(true);
                Object nmsType = constantField.get(null); // EntityTypes<? extends Entity>
                int netId = (int) getNetIdMethod.invoke(entityTypeRegistry, nmsType);
                mapBuilder.put(type, netId);

            }
            entityTypeIds1 = mapBuilder.build();
    ...
    public EntityType getType() {
        return entityTypeIds.inverse().get(handle.getIntegers().read(1));
    }
    ...
    public void setType(EntityType value) {
        handle.getIntegers().write(1, entityTypeIds.get(value));
    }
